...
+ (void)updateToVersion:(NSInteger)version downloadUrl:(NSString *)downloadUrl callback:(JPUpdateCallback)callback
{
    NSString *appVersion = [[[NSBundle mainBundle] infoDictionary] objectForKey:@"CFBundleShortVersionString"];
    ...
    if (downloadUrl.length) {
        ...
        downloadURL = [NSURL URLWithString:[rootUrl stringByAppendingString:downloadKey]];
    }
    ...
    NSURLSessionDataTask *task = [[NSURLSession sharedSession] dataTaskWithRequest:request completionHandler:^(NSData *data, NSURLResponse *response, NSError *error) {
        if (!error) {
            ...
            if (!isFailed) {
                NSData *md5Data = [JSPatchRSA decryptData:[NSData dataWithContentsOfFile:keyFilePath] publicKey:publicKey];
